...
export const verifySignature = async (
  ...
  const jwtResponse = await axiosInstance.post(
    `/users/${address}/verify-signature`,
    {
      address,
      isSmartWallet,
      domain,
...
export const commitToBuy = async (supplyId, data, token) => {
  const voucherIdRawResult = await axiosInstance.post(
    `/vouchers/commit-to-buy/${supplyId}`,
    data,
    {
      headers: { Authorization: `Bearer ${token}` },
    }
...
export const getAccountVoucherSets = async (account) => {
  const accountVoucherSets = await axiosInstance.get(
    `/voucher-sets/sell/${account}`
  );
  return accountVoucherSets.data;
};

export const getVouchersFromSupply = async (address, token) => {
  const vouchersFromSupply = await axiosInstance.get(
    `/vouchers/buyers/${address}`,
    {
      headers: { Authorization: `Bearer ${token}` },
    }
  );
...
export const getThread = async (data, token) => {
