...
function ItemsListingModal({ items, setModal }) {
  ...
  return (
    <section
      className="items-listing-background"
      onClick={(e) => {
        if (e.target.className === "items-listing-background") {
          setModal(false);
        }
      }}
    >
      <section className="items-listing-modal">
        ...
        <p class="next" onClick={onNext} onKeyUp={onNext}>
          ❯
        </p>
        <p class="prev" onClick={onPrevious} onKeyDown={onPrevious}>
          ❮
        </p>
      </section>
    </section>
